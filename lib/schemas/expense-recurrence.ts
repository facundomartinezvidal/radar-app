/**
 * Zod schemas for expense recurrence input shapes.
 *
 * Recurrences define a repeating expense pattern.
 * Computed fields (day_of_month, next_run_on) are NOT included — they are
 * derived server-side and must not be sent by the client.
 *
 * Frequency primitives are re-exported from income-recurrence to avoid
 * duplication — both feature sets share the same four frequencies.
 */
import { z } from 'zod';

export { FREQUENCIES, type Frequency } from './income-recurrence';
import { FREQUENCIES } from './income-recurrence';

// ---------------------------------------------------------------------------
// Create
// ---------------------------------------------------------------------------

export const createExpenseRecurrenceSchema = z
  .object({
    amount: z
...
export type CreateExpenseRecurrenceInput = z.infer<typeof createExpenseRecurrenceSchema>;

// ---------------------------------------------------------------------------
// Update — all fields optional, end >= start refine still enforced when both present
// ---------------------------------------------------------------------------

export const updateExpenseRecurrenceSchema = z
  .object({
    amount: z
      .number({ message: 'Ingresá un monto válido.' })
      .positive('El monto tiene que ser mayor a cero.')
...
export type UpdateExpenseRecurrenceInput = z.infer<typeof updateExpenseRecurrenceSchema>;
