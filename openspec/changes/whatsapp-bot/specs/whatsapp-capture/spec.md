...
A linked user SHALL be able to register an expense or income by sending free-form text. The webhook SHALL classify intent and extract entities (amount, currency, merchant/description, date, category hint, direction) with a Groq text model, and map them to a transaction row using the existing `ImportTransactionRow` shape.
...
The webhook SHALL accept audio messages, resolve the Cloud API media `id` to a download URL via the Graph API and fetch the bytes using the `WHATSAPP_ACCESS_TOKEN` bearer, transcribe with Groq Whisper `whisper-large-v3`, and feed the transcript into the text-capture pipeline.
...
- **WHEN** the Cloud API media cannot be resolved or fetched
