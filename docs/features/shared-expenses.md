...
**Test baseline after this feature:** 834 tests, 64 suites
...
| 3   | Invite registered users by exact email (consent model)                  | Shipped |
...
| 6   | Choose who paid (any active member or placeholder)                      | Shipped |
| 7   | Split by equal shares, custom amounts, or percentages                   | Shipped |
| 8   | View net balances per member, separated by currency                     | Shipped |
| 9   | Mark a debt as settled (`group_settlements`)                            | Shipped |

### Non-functional

- **Security:** Registered users only enter splits/balances after accepting. `is_group_member()` SECURITY DEFINER prevents RLS recursion. `invite_group_member()` SECURITY DEFINER enables email lookup in `auth.users` (the only intentional advisor lint — see ADR).
...
| ----------------------- | ----------- | ----------------------------------------------------------------------- |
| `create_group`          | INVOKER     | Group + owner member + placeholders in one transaction                  |
...
- **`lib/split-math.ts`**: `computeShares(amount, members, mode, values?)` — equal/custom/percent with cent-level remainder to last member. `simplifyDebts(balances)` — greedy pairwise, per currency.
...
- **`lib/repositories/groups.ts`** + **`lib/repositories/shared-expenses.ts`**: typed wrappers over RPCs.
...
| Component                                     | Purpose                                           |
| --------------------------------------------- | ------------------------------------------------- |
| `components/groups/group-form.tsx`            | Name + icon + color form with live preview chip   |
...
| `components/groups/split-editor.tsx`          | Three-mode split UI with running-total validation |
| `components/groups/balance-row.tsx`           | Per-member balance row (green/red/neutral)        |
| `components/groups/member-selector-sheet.tsx` | Bottom sheet to pick "who paid"                   |
...
- **Re-split of existing shared expense** — MVP: delete + recreate. Split ids are not stable (same pattern as `expense_items`).
