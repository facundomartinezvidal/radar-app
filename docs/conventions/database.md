...
- **WhatsApp bot service RPCs follow Pattern 1** (`redeem_link_code`, `resolve_wa_user`, `unlink_wa`, `import_transactions_for`, `get_personal_totals_for`, `get_expense_by_category_for`, `get_expense_by_period_for`, `get_income_by_period_for`): SECURITY DEFINER, REVOKE EXECUTE from `anon`, `public`, `authenticated`. Called only by the `whatsapp-webhook` edge function running with the service-role key. These accept an explicit `p_user_id uuid` parameter in place of `auth.uid()` — a caller-supplied uid pattern that requires Pattern 1 enforcement to be safe. Supabase advisor does not flag these. See `docs/decisions/2026-06-21-whatsapp-bot-schema.md`.
