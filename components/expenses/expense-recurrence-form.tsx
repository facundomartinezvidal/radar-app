/**
 * Shared form for expense recurrence create/edit.
 *
 * Drives react-hook-form + zod on `createExpenseRecurrenceSchema`. Fields:
 * - amount       → AmountInput (red, expense colour)
 * - currency     → CurrencyToggle
 * - category_id  → CategoryPicker (kind='expense')
 ...
 *
 * Props mirror RecurrenceForm (income) for consistency.
 */
...
import type { CategoryRow, ExpenseRecurrenceWithCategory } from '@/lib/repositories/expenses';
...
export interface ExpenseRecurrenceFormProps {
  categories: CategoryRow[];
  /** Optional initial values (edit mode — DB row). */
  initial?: ExpenseRecurrenceWithCategory | null;
  onSubmit: (input: CreateExpenseRecurrenceInput) => Promise<void> | void;
  submitLabel?: string;
...
interface ExpenseRecurrenceFormInternalFields {
  amount: number;
  amountText: string;
  currency: Currency;
          ...
          >
            <View
              style={{
                ...
                backgroundColor: selected ? colors.money.out : 'transparent',
              }}
            >
              <Text
                variant="bodySm"
...
export function ExpenseRecurrenceForm({
  categories,
  initial,
  onSubmit,
  submitLabel = 'Crear gasto recurrente',
  isSubmitting = false,
  submitError = null,
}: ExpenseRecurrenceFormProps): React.JSX.Element {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<ExpenseRecurrenceFormInternalFields>({
    // Cast: we add `amountText` as a UI-only field; zod validates the numeric `amount`.
    resolver: zodResolver(createExpenseRecurrenceSchema) as never,
    mode: 'onChange',
    defaultValues: {
        ...
        >
          ...
          <View
            style={{
              ...
              backgroundColor: indefinite ? colors.money.out : colors.bg[3],
              borderWidth: 1,
              borderColor: indefinite ? colors.money.out : colors.line[2],
