/**
 * Tests for ExpenseRecurrenceForm — verifies fields render in create/edit mode,
...
import { ExpenseRecurrenceForm } from '../expense-recurrence-form';
...
describe('ExpenseRecurrenceForm — field rendering', () => {
  it('renders the Monto label', () => {
    render(<ExpenseRecurrenceForm categories={CATEGORIES} onSubmit={jest.fn()} />);
  ...
  it('renders the description input with the correct placeholder', () => {
    render(<ExpenseRecurrenceForm categories={CATEGORIES} onSubmit={jest.fn()} />);
    expect(screen.getByPlaceholderText('Descripción del gasto recurrente')).toBeTruthy();
  });
  ...
  it('renders the default submit label "Crear gasto recurrente"', () => {
    render(<ExpenseRecurrenceForm categories={CATEGORIES} onSubmit={jest.fn()} />);
...
describe('ExpenseRecurrenceForm — submit behaviour', () => {
  it('calls onSubmit with parsed amount and defaults when submitted with a valid amount', async () => {
    const onSubmit = jest.fn().mockResolvedValue(undefined);
    render(<ExpenseRecurrenceForm categories={CATEGORIES} onSubmit={onSubmit} />);
...
describe('ExpenseRecurrenceForm — submitError', () => {
  it('shows submitError when provided', () => {
    render(
      <ExpenseRecurrenceForm
        categories={CATEGORIES}
        onSubmit={jest.fn()}
        submitError="No se pudo crear el gasto recurrente. Intentá nuevamente."
      />,
    );

    expect(
      screen.getByText('No se pudo crear el gasto recurrente. Intentá nuevamente.'),
    ).toBeTruthy();
  });

  it('does NOT show submitError when null', () => {
    render(
      <ExpenseRecurrenceForm categories={CATEGORIES} onSubmit={jest.fn()} submitError={null} />,
    );
    expect(
      screen.queryByText('No se pudo crear el gasto recurrente. Intentá nuevamente.'),
    ).toBeNull();
  });
});
