...
- `whatsapp_links`, `whatsapp_link_codes`, `whatsapp_messages`, `whatsapp_conversations` — identity bridge + idempotency + confirmation state for the WhatsApp bot; 12 RPCs (1 INVOKER + 11 Pattern-1 DEFINER `_for` variants incl. `get_recent_movements_for` for list/detail queries and `get_income_totals_for`/`get_income_by_category_for` for income totals/net/breakdown, revoked from `authenticated`) (HU-26..29) → [`docs/decisions/2026-06-21-whatsapp-bot-schema.md`](docs/decisions/2026-06-21-whatsapp-bot-schema.md), [`docs/decisions/2026-06-22-whatsapp-query-llm-render.md`](docs/decisions/2026-06-22-whatsapp-query-llm-render.md)
...
- WhatsApp bot — **Twilio transport variant** (live; supersedes Meta Cloud API for delivery — Meta test mode does not deliver to AR numbers): same schema/RPCs/logic, swaps transport only (`X-Twilio-Signature` HMAC-SHA1, form-encoded inbound, Twilio Messages API reply, Basic-auth media fetch, empty-TwiML ack). Adds recent-movements listing (`get_recent_movements_for`, "mis últimos N gastos"/"detalle del último") + conversational `chat` intent (Groq Q&A over the user's totals/categories/movements). Secrets: `TWILIO_ACCOUNT_SID`/`TWILIO_AUTH_TOKEN`/`TWILIO_WHATSAPP_FROM` → [`docs/decisions/2026-06-22-whatsapp-twilio-transport.md`](docs/decisions/2026-06-22-whatsapp-twilio-transport.md)
- WhatsApp bot — **LLM-rendered query experience** (HU-28): all totals/category/chat replies rendered by Groq into natural rioplatense Spanish (moderate emojis) over a shared `finance_context.ts` assembler; complete coverage (income totals, net balance, income-by-category, month-over-month, period-aware chat). LLM never computes figures — exact pre-formatted figures + `render.ts` verbatim-number guard + deterministic template fallback on any Groq failure. Adds `get_income_totals_for`/`get_income_by_category_for` RPCs. `listMode` stays deterministic (Deno edge-fn tests 193 → 241; jest suite unaffected) → [`docs/features/whatsapp-query-llm-render.md`](docs/features/whatsapp-query-llm-render.md), [`docs/decisions/2026-06-22-whatsapp-query-llm-render.md`](docs/decisions/2026-06-22-whatsapp-query-llm-render.md)
