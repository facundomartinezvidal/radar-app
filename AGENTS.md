...
RADAR uses pg_cron for the income recurrence materializer (`'materialize-due-incomes'`, `'0 6 * * *'`). Key rules:

- **Materialization functions follow Pattern 1** (`materialize_due_incomes`, `advance_occurrence`): SECURITY DEFINER, REVOKE EXECUTE from `anon`, `authenticated`, `public`. Called only by pg_cron (running as the `postgres` role). Supabase advisor does not flag Pattern 1.
- **`advance_occurrence` is IMMUTABLE, NOT STRICT**: pure date arithmetic, no session-state calls. NOT STRICT because `p_dom` is legitimately NULL for weekly/biweekly/yearly rules — STRICT would short-circuit the entire function to NULL on any NULL argument, breaking non-monthly frequencies.
- **Idempotency via `occurred_date DATE` column**: `timestamptz::date` is STABLE (session-timezone dependent) and cannot appear in a btree expression index. The `incomes.occurred_date` plain DATE column avoids this; the unique index `(recurrence_id, occurred_date) WHERE recurrence_id IS NOT NULL` is a standard non-expression btree index. Manual (one-off) incomes leave `occurred_date = NULL` and are excluded from the uniqueness constraint.
- **Local dev without pg_cron**: run `select public.materialize_due_incomes();` manually via Supabase MCP to simulate the cron job.
...
pnpm test           # jest-expo + RNTL (1276 tests, 82 suites baseline)
```
